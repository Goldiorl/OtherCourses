...
public class Graph {
	...
	//-------dfs-----------
		...
		public int dfsMst(){
			...
			int dfsIndex=0;
			//System.out.println("null??:"+ (vertex==null));
			vertex.isVisted=true;
			dfs[dfsIndex++]=vertex;
			...
			int rootreturn=0;
			if (LargestSize==0) rootreturn=0;
			else if (LargestSize==1) rootreturn=retValueList.get(0);
			else if (LargestSize>1) rootreturn=retValueList.get(LargestSize-1)+retValueList.get(LargestSize-2);
			if(rootreturn>=maximum_diameter) return rootreturn;
			else return maximum_diameter;
		...
		public int exploreMst(MstVertex vertex,int sourceindex,int dfsIndex){
			...
			while(nextVertex!=null){
				...
				if((vertex.maxlength+returnedlength)>maximum_diameter) maximum_diameter=vertex.maxlength+returnedlength;
				nextVertex=(MstVertex) getAdjVertex(vertex);
			}
			//System.out.println("Source index is "+sourceindex);
			//System.out.println("This vertex is "+vertex.index);
			//vertex.maxlength+=searchEdgeMap.get(new Pair<Integer,Integer>(Math.min(sourceindex,vertex.index),Math.max(sourceindex,vertex.index))).weight;
			vertex.maxlength+=1;
			//System.out.println("Im returnning "+vertex.maxlength +"from" +vertex.index);
	...
	public static void main(String args[]){
		...
		//-------Run code----------------for deliverable 1
				...
				for(int nvalueindex=2;nvalueindex<3;nvalueindex++){
					...
					for(double prob=0.02;prob<1;prob+=0.02){
						...
						for(int times=0;times<500;times++){
							...
							for (int index=0;index <ccnumforgraph;index++){
								...
								if(edgesToSort.length==0) continue;
								Kruscal kruscal=new Kruscal(edgesToSort);
								kruscal.findMST();

								
								diameter+=kruscal.getDiameter();

							}//for ccnumforparagraph
							

							diameteraccum+=((double)diameter)/((double)ccnumforgraph);

				
						}//times
						

						double result=(((double)diameteraccum)/((double)trial));
						data[nvalueindex][pindex]=result;

						System.out.println("average diameter is"+result);
					...
					try
			        {
			            PrintWriter pr = new PrintWriter(nvalueindex+"file_averagediameter");    
...
class Kruscal{
	...
	public int getDiameter(){
		...
		for (Edge e:mst){
			if(e==null) break;
			//System.out.println("Print out e.start :" +e.start);
