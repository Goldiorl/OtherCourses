...
(* folded function, base, should be same type, say _t
   xs is a list of _t, 
   f a x: a is of type _t, derived from base, x is _t,  and f a x returns _t
*)
...
let pipe fs = 
  let f a x = fun p_rec -> x (a p_rec) in
  let base = fun p -> p in
    List.fold_left f base fs

(* 
an example of func regenereted by recursive parameter

f base h:tl
fold_left f (f base h) tl
f base h -> ('a -> 'a)
pipe : ('a -> 'a ) list -> ('a -> 'a)
*)
...
let padZero l1 l2 = let _diff = List.length(l1) - List.length(l2) in 
    match _diff < 0 with
      | true -> ( ((clone 0 (-_diff)) @ l1), l2)
      | false -> (l1, ((clone 0 _diff) @ l2))
(* UNCOMMENT AFTER IMPLEMENTING THE ABOVE
...
let bigAdd l1 l2 = 
  let add (l1, l2) = 
    let f a x = 
      ...
      let (new_param1, new_param2) = x in
        ( (carry + new_param1 + new_param2)/10, ( (carry + new_param1 + new_param2) mod 10 )::result )
    in
    let base = (0, []) in 
    let args = let r1 = (List.rev (0::l1)) in let r2 = (List.rev (0::l2)) in (List.combine r1 r2) in
...
(* The tuple is (carry, ['result'])
   f returns (carry, ['result']) == 
   based = (0, [])   == a 
   args is a list of x, 
   x is 

   add(l1, l2) returns the res
   base =(0, 0) 
   f a x  => return the tuple
*)
...
let rec mulByDigit i l = 
  if i<=0 then []
    ...
    let rec helper_rec carry rem_l acc_res_l = match rem_l with
      | [] -> if carry = 0 then acc_res_l else carry::acc_res_l
      | h::tl-> let h_process =((i*h+carry))mod 10 in 
          let new_carry = (i*h +carry)/10 in
            helper_rec new_carry tl (h_process::acc_res_l)
    in
      helper_rec 0 (List.rev l) []
...
let bigMul l1 l2 = 
  let f a x = let ( l1, formal_result ) = a in 
    let mul_result = mulByDigit x l1 in 
    let this_result = bigAdd (formal_result @ [0]) mul_result in
      (l1, this_result)
  in
  let base = ( l1 , [0] ) in
  let args = l2 in
