...
let rec digitsOfInt n = 
  let rec aux n acc = match n<=0 with 
    |true -> acc
    |false -> aux (n/10) ( (n mod 10)::acc )
  in aux n []
...
let rec additivePersistence n = match abs(n)<10 with
  | true -> 0
  | _ -> additivePersistence( sumList(digits n) ) +1
...
let rec digitalRoot n = match abs(n)<10 with
  | true -> n
